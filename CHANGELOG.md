...
### Fixed
- **Guild Found was switched off for the first few seconds of every login.** Your client doesn't know which guild you're in until the roster loads, and until then it fell back on the no-guild settings — where every restriction is off by default. Trade, mail and the Auction House were all unrestricted during that window. It now uses the last guild the character was in until the real one resolves, so the closed economy is enforced from the moment you log in. One side effect, deliberately on the safe side: for the first seconds a trade with a genuine guildmate may show the Trade button disabled, because the roster needed to confirm they're a guildmate. It re-enables as soon as the roster lands.
- **Other addons could silently remove the trade, mail and Auction House blocks.** These work by wrapping the game's own functions, and they were only installed once at login — so any addon that loaded afterwards and replaced one of those functions quietly disabled the protection, leaving only a greyed-out Trade button with nothing behind it. All of them are now re-applied whenever you open a trade, your mailbox, or the Auction House, so the addon stays the outermost check. (The mail attachment blocks already worked this way; the rest now match.)
- **The played-without-addon tolerance now also applies to the gold/item check.** They were separate, so a member well inside the tolerance a leader had deliberately set could still come back flagged for value moved in that same window — one event producing two roster rows that contradicted each other. The leader's setting now governs both.

### Changed
- **With no tolerance configured, the gold/item check now allows none at all.** Previously it ignored addon-off windows shorter than 5 seconds. It now checks *any* window, however short. This costs nothing in false alarms: a window in which nothing actually moved still reports nothing, and small amounts stay below the reporting threshold — it just removes a small blind spot where a very quick disable/re-enable was never even looked at. If your guild leader has set a played-without-addon tolerance, that setting still governs.
- **Guild bank alts could accept outside gold without being flagged.** Reported from a live test: disable the addon, accept gold from a non-guild member, re-enable — still compliant. The wealth check offsets any rise against the value in your bank, on the theory it might be something you withdrew rather than something new. But Classic has no bank gold, so gold can never have come from there — and on a bank alt that offset is enormous, so outside gold was absorbed up to the value of the bank's contents. Gold is no longer offset by the bank at all. **Mail from a fellow guild member still offsets both gold and items** (that's value moving inside the guild); mail from anyone outside the guild — including the Auction House and system mail — offsets nothing, so gold and items arriving that way now flag.
- **Items are now checked against your bank instead of assumed against it.** The addon can't see your bank at login — only while the bank window is open — so it used to just assume a rise in what you carry might have come from there, which on a bank alt excused almost anything. Now it holds the question open: the value isn't counted against you, and the next time you open your bank it checks whether the bank actually went down by that much. What the bank accounts for is dropped; what it can't is counted. If you never open your bank again, it counts after 8 hours of played time, so avoiding the bank isn't a way out. `/sodlock wealth` shows anything pending.
- **Guildmate mail offsets are used up instead of applying forever.** The same mail balance was re-credited on every addon-off window, making one large delivery a standing allowance rather than a one-off explanation. It's now consumed as it's used, and re-measured next time you open your mailbox.
- **Opening your mailbox before it finished loading could wipe your mail offset.** The inbox reads as empty for a moment when it opens, and that empty reading was being saved — removing an offset you'd legitimately earned and making a later, honest mail collection look like unexplained gain. The mailbox now waits for real data, matching what the bank check already did.
- **"Over level cap" no longer sticks after the level rule is turned off.** Every other check clears its own result when its rule is disabled; the level check returned early and left the flag latched, so a member who had been over cap kept reporting "over level cap" on the compliance roster for the rest of the session even after the rule was switched off. It now clears like the rest.
- **The played-without-addon tolerance can no longer be set to a value that silently disables it.** A tolerance of 0 minutes read as "flag any gap" but was interpreted everywhere as "check is off" — the strictest-looking setting turned the feature off entirely. The slider's minimum is now the smallest gap the detector can actually tell apart from ordinary login/logout processing and a client crash, and any existing config saved below that floor now behaves as (and displays as) that floor. **If your guild has this check on with a tolerance under 3 minutes, members who play without the addon will start being flagged after this update — previously nothing was.**
- **The welcome window no longer tells guild members they are the guild leader.** It opened a second after login, when the client often hasn't loaded your guild yet — and "no guild" is treated as "you control your own config", so the notice claimed leader status for ordinary members. It now waits for your guild to resolve, and re-checks the role each time it opens.
- **A new alt no longer gets asked to choose a mode your guild has already chosen.** The Relaxed/Authentic picker now appears only on a genuine first run — when nobody has set a phase yet. Once your guild is configured, a new character gets a read-only summary instead (phase, mode, what's enforced, next unlock date) with a "Got it" button. This also closes a serious hole: rolling a new alt as **guild leader** used to show the picker, and clicking "Play Relaxed" on it silently wiped the guild's entire enforcement config and broadcast the change to every member.

### Added
- **The Audit window shows which items appeared again.** Since 0.7.9 it reported only an estimated total ("~40g moved while unmonitored"), which couldn't tell an officer whether that was one raid drop or forty herbs — the per-item list went away with the wealth-tracking rewrite. It's back, as a list of the items that arrived across the unmonitored window, with icons and tooltips. Important: it is **evidence to read, not a verdict**. Flagging is still decided purely by the estimated value, so ordinary looting and questing showing up in this list costs nothing — which is what lets the list exist without bringing back the false-positive problems the 0.7.9 rewrite removed. The list is bounded, only sent for members who are actually flagged, and is cleared along with the counter when an officer clears them.

- **The compliance roster now tells officers when a member's wealth check isn't live yet.** Guild Found wealth integrity can't judge a character until their addon has seen their bank and mailbox at least once (see the fix above), and during that window they report clean whether or not they are. Officers now see "(wealth check not yet armed)" against those members instead of nothing. It is a note, not a violation — the member has done nothing wrong and can't speed it up — so it never affects their compliant/out-of-compliance status. Members on an older version show nothing, as before.
- **Guild Found wealth integrity: a first bank or mailbox withdrawal across an addon-off window is no longer flagged as outside income.** Your bank and mailbox can only be read while their window is open, and until the addon has seen each one at least once it has no idea what's in them. It was treating "never seen" as "empty", so the first withdrawal after installing looked like value that appeared from nowhere — worst for members with a well-stocked bank. Value that can't be attributed is now left alone rather than reported. `/sodlock wealth` shows whether each has been seen yet.
